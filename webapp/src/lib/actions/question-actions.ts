...
export async function getQuestions(tag?: string): Promise<FetchResponse<Question[]>> {
    ...
    if (!questions || questionError) {
        return {
            data: null,
            error: {
                message: 'Problem setting questions',
                status: 500
            }
        }
    }
    ...
    const profilesUrl = '/profiles/batch?' 
        + new URLSearchParams({ids: ids.join(',')});
    const {data: profiles, error: profilesError} 
        = await fetchClient<Profile[]>(profilesUrl, 'GET', 
...
export async function getQuestionById(id: string): Promise<FetchResponse<Question>> {
    ...
    if (userIds.size === 0) {
        return {data: null, error: {message: 'Problem getting userIds', status: 500}};
    }
